    ...
    const { data, error } = await supabase
      ...
      .select(
        `id, quantity,
         ...
         book:books(id, name, image, price)` // <-- replaced add_products with books, removed years & size
      )
          ...
          <button
            onClick={() => {
              if (confirm("Delete ALL orders?"))
                checkouts.forEach((c) => deleteCheckout(c.checkout.id));
            }}
            style={{
              backgroundColor: mainColor.button_hex,
              color: mainColor.text_color,
          ...
          <thead className="bg-pink-100 text-gray-700">
            <tr>
              {[
                ...
                <th key={title} className="px-2 sm:px-4 py-2 text-left">
                  {title}
                </th>
              ))}
            </tr>
            ...
            {groupedCheckouts.map((checkout, orderIndex) => (
              <>
                <tr className="bg-gray-100">
                  <td
                    colSpan={12}
                    className="px-2 sm:px-4 py-2 font-semibold text-gray-700 truncate"
                  >
                    Order #{orderIndex + 1} — {checkout.first_name}{" "}
                    {checkout.last_name}
                  </td>
                </tr>
                ...
                {checkout.items.map((item: any, index: number) => (
                  ...
                  >
                    {index === 0 && (
                      <>
                        <td rowSpan={checkout.items.length}>
                        ...
                        <td rowSpan={checkout.items.length}>{checkout.city}</td>
                        <td rowSpan={checkout.items.length}>
                          {checkout.region}
                        </td>
                      </>
                          ...
                          <div className="flex flex-col sm:flex-row items-center gap-2 justify-center flex-wrap">
                            ...
                            <div className="flex flex-wrap gap-2">
                              <button
                                ...
                                className="px-2 sm:px-3 py-1 bg-red-500 text-white rounded-md hover:bg-red-600 transition"
                              >
                                Delete
                              </button>
                              <button
                                onClick={() => generateInvoice(checkout)}
                                className="px-2 sm:px-3 py-1 bg-green-500 text-white rounded-md hover:bg-green-600 transition"
                              >
                                PDF
                              </button>
                              <Link
                                href={`/admin/checkouts/EditCheckoutPage/${checkout.id}`}
                                className="px-2 sm:px-3 py-1 bg-blue-500 text-white rounded-md hover:bg-blue-600 transition"
                              >
                                Edit
                              </Link>
                            </div>
